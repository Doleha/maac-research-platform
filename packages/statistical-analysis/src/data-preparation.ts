...
export function buildStandardizedMatrix(
  ...
  const standardizedMatrix: number[][] = Array(numRows).fill(null).map(() => Array(numCols).fill(0));
  ...
  for (let col = 0; col < numCols; col++) {
    ...
    const variance = colValues.reduce((sum, val) => sum + Math.pow(val - mean, 2), 0) / (numRows - 1);
...
export function buildBatchPayload(
  experiments: ExperimentData[],
  config: Tier2Config = DEFAULT_TIER2_CONFIG
): BatchPayload {
  ...
  const primaryData = dimensionalData['maac_overall_score'] || 
    experiments.map(e => e.maac_overall_score);
  ...
  const calls: StatisticalMethodCall[] = [
    ...
    { id: `maac_scoring_validation:${shortId()}`, method: 'maac_scoring_validation', params: { scores: matrixData } },
    { id: `maac_dimensional_statistics:${shortId()}`, method: 'maac_dimensional_statistics', params: { data: dimensionalData } },
    { id: `maac_framework_coherence:${shortId()}`, method: 'maac_framework_coherence', params: { scores: matrixData } },
    { id: `maac_multivariate_validation:${shortId()}`, method: 'maac_multivariate_validation', params: { data: dimensionalData } },
    
    // Correlation analysis
    { id: `corr_matrix:${shortId()}`, method: 'corr_matrix', params: { X: standardizedMatrix } },
    { id: `cronbach_alpha:${shortId()}`, method: 'cronbach_alpha', params: { X: standardizedMatrix } },
    { id: `pearson:${shortId()}`, method: 'pearson', params: { 
      X: primaryData, 
      Y: dimensionalData['maac_content_quality'] || primaryData 
    }},
    { id: `spearman:${shortId()}`, method: 'spearman', params: { 
      X: primaryData, 
      Y: dimensionalData['maac_content_quality'] || primaryData 
    }},
    ...
    { id: `bartlett_sphericity:${shortId()}`, method: 'bartlett_sphericity', params: { X: standardizedMatrix } },
    { id: `pca:${shortId()}`, method: 'pca', params: { X: standardizedMatrix } },
    { id: `efa:${shortId()}`, method: 'efa', params: { X: standardizedMatrix, n_factors: 2 } },
    
    // Effect sizes
    { id: `cohens_d:${shortId()}`, method: 'cohens_d', params: { 
      x: primaryData, 
      y: dimensionalData['maac_content_quality'] || primaryData 
    }},
    { id: `hedges_g:${shortId()}`, method: 'hedges_g', params: { 
      x: primaryData, 
      y: dimensionalData['maac_content_quality'] || primaryData 
    }},
    
    // Bootstrap methods
    { id: `bootstrap_ci:${shortId()}`, method: 'bootstrap_ci', params: { X: primaryData, confidence_level: 0.95 } },
    { id: `bootstrap_bca:${shortId()}`, method: 'bootstrap_bca', params: { X: primaryData, confidence_level: 0.95 } },
    
    // Reliability analysis
    { id: `split_half:${shortId()}`, method: 'split_half', params: { X: matrixData } },
    { id: `item_total_corr:${shortId()}`, method: 'item_total_corr', params: { X: standardizedMatrix } },
    
    // Power analysis
    { id: `power_ttest:${shortId()}`, method: 'power_ttest', params: { effect_size: 0.5, nobs: experimentCount } },
    { id: `power_anova:${shortId()}`, method: 'power_anova', params: { 
    ...
    { id: `outlier_detection_robust:${shortId()}`, method: 'outlier_detection_robust', params: { data: primaryData } },
    
    // Mediation analysis (if enough dimensions)
    { id: `mediation_analysis:${shortId()}`, method: 'mediation_analysis', params: {
...
export function validateExperimentData(experiments: ExperimentData[]): {
  ...
  if (dimensionsPresent.length < 3) {
    errors.push(`Insufficient MAAC dimensions: ${dimensionsPresent.length} present, minimum 3 required`);
  }
...
export function sanitizeForPythonEngine(experiments: ExperimentData[]): ExperimentData[] {
  return experiments.map(exp => {
    ...
    return sanitized as ExperimentData;
  });
}
