...
export class MAACEvaluator {
  ...
  constructor(config: MAACEvaluatorConfig) {
    ...
    this.assessors = createAllAssessors(
      config.llmProvider,
  ...
  calculateDerivedMetrics(
    ...
    const factualClaims = sentences.filter(s => 
      /\d+/.test(s) || 
      /%/.test(s) || 
      /\b(is|are|was|were|will be|has been)\b/.test(s)
    ).length;
    ...
    const solutionSteps = (text.match(/\b(step|first|second|third|finally|then|next)\b/gi) || []).length;
    ...
    return {
      ...
      // Tool efficiency
      cognitiveCyclesWithTools: metadata.toolsInvokedCount > 0
        ? Math.floor(metadata.cognitiveCyclesCount * 0.7)
        : 0,
      toolEnhancedProcessing: metadata.toolsInvokedCount > 0
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━`;
  ...
  private extractDimensionReasonings(
    ...
    const extractReasoning = (dimension: MAACDimension): string => {
      ...
      return score.keyObservations.join('. ') || 
