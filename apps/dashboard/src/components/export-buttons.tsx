...
export function ExportButtons({ experimentId, experimentName }: ExportButtonsProps) {
  ...
  const getButtonClass = (format: ExportFormat) => {
    const baseClass = "inline-flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed";
    ...
    <div className="space-y-4">
      ...
      <div className="text-xs text-gray-500">
        ...
        <ul className="space-y-0.5 ml-4">
          <li>• <span className="font-medium">CSV</span> - Trial data with MAAC scores</li>
          <li>• <span className="font-medium">JSON</span> - Full experiment data structure</li>
          <li>• <span className="font-medium">PDF</span> - Formatted report with visualizations</li>
        </ul>
      </div>
    </div>
  );
